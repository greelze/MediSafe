...
scales: { 
        x: { 
          title: {
            display: true,
            text: 'Gathered Reading per seconds',
...
function updateStats() {
  ...
  try {
    localStorage.setItem('medisafeAnalyticsAlertTotals', JSON.stringify({
      totalAlerts,
      temperatureAlerts: tempAlerts,
      humidityAlerts,
      uvAlerts: lightAlerts
    }));
  } catch (e) {
    console.warn('Unable to persist analytics totals:', e);
  }
}
...
function buildCSV(mode = 'all') {
  ...
  const rows = filteredData.flatMap(row => {
    const date = new Date(row.recorded_id).toLocaleDateString();
...
window.deleteLocation = function(id, name) {
  ...
  if (dModal && dName) {
    ...
    dModal.style.setProperty('display', 'flex', 'important'); // open
  }

  document.getElementById('confirmDelete').onclick = async () => {
    const { error } = await supabase.from('location').delete().eq('id', id);
    if (!error) {
      dModal.style.setProperty('display', 'none', 'important'); // ← fix: use setProperty here too
      loadDashboardData();
    } else {
  ...
  document.getElementById('cancelDelete').onclick = () => {
    dModal.style.setProperty('display', 'none', 'important'); // ← fix: same here
  };
};
